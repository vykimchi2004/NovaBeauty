...
export default function ComplaintManagementPage() {
    ...
    const handleTransferAdmin = () => {
        ...
        // Kiểm tra ghi chú trước - không cần popup, chỉ hiển thị lỗi ngay dưới ô ghi chú
        if (!note || !note.trim()) {
                                            ...
                                            <tr key={complaint.id}>
                                                ...
                                                <td>
                                                    <span
                                                        className={cx(
                                                            'status',
                                                            `status-${complaint.statusRaw?.toLowerCase() || complaint.status
                                                                .toLowerCase()
                                                                .replace(/\s+/g, '-')}`,
                                                        )}
                                                    >
                ...
                <div className={cx('modal-overlay')} onClick={handleCloseModal}>
                    <div className={cx('modal-content')} onClick={(e) => e.stopPropagation()}>
                        <div className={cx('modal-header')}>
                            <h2 className={cx('modal-title')}>Chi tiết khiếu nại</h2>
                            <button className={cx('modal-close-btn')} onClick={handleCloseModal}>
                                ×
                            </button>
                        </div>
